...
async function onConfigChange(config) {
  ...
  // Update text content (safe)
  setTextIfExists("brand-name", brandName);
  setTextIfExists("hero-title", heroTitle);
  setTextIfExists("hero-description", heroDescription);
  setTextIfExists("mission-headline", missionHeadline);
  setTextIfExists("mission-description", missionDescription);
  ...
  // Update font sizes (safe checks)
  const heroTitleEl = document.querySelector(".hero-title");
  if (heroTitleEl) heroTitleEl.style.fontSize = `${baseSize * 2.5}px`;

  const heroTextEl = document.querySelector(".hero-text");
  if (heroTextEl) heroTextEl.style.fontSize = `${baseSize * 1.1}px`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCartUI() {
    ...
    if (itemCount === 0) {
      cartItemsEl.innerHTML =
        '<li style="text-align: center; color: #666; border: none; background: transparent;">Your cart is empty</li>';
    }
  }
});
...
function showPolicy(policyType) {
  if (!policyModal || !modalTitle || !modalContent) return;
...
if (closeModal) closeModal.addEventListener("click", hidePolicy);
