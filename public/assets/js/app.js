...
let modalDepth = 0;
let suppressNextPopHandler = false;
...
function syncHistoryOnClose(closeFn) {
if (!isMobile() || !history.state?.__faideModal) return false;
suppressNextPopHandler = true;
closeFn();
history.back();
setTimeout(() => {
  suppressNextPopHandler = false;
}, 80);
return true;
}
...
const paypalLink = $("checkout-paypal-link");
...
function syncTotals() {
  ...
  if (paypalLink) paypalLink.href = `https://www.paypal.com/paypalme/faideClothing/${total.toFixed(2)}`;
}
...
function close(fromPop = false) {  
  if (!fromPop && syncHistoryOnClose(() => close(true))) return;
  closeModal(overlay);  
}  
...
return isProductInStock(product) ? product.label || "New" : "Out of Stock";
}
...
products.forEach((p) => {  
  ...
  card.innerHTML = `  
    ...
    <div class="product-info">  
      <div class="product-label${isProductInStock(p) ? "" : " out-of-stock-label"}">${getProductLabel(p)}</div>  
      ...
      <div class="options" aria-label="Product options">  
        ...
        ${isProductInStock(p) ? `<button type="button" class="secondary-btn add-to-cart" disabled>Add to Bag</button>` : `<button type="button" class="secondary-btn add-to-cart" disabled>Out of Stock</button><div class="stock-note">Set <code>"inStock": true</code> in products.json when this item is available again.</div>`}  
      </div>  
    </div>  
  `;  
...
function closeDrawer(fromPop = false) {  
  ...
  if (!fromPop && syncHistoryOnClose(() => closeDrawer(true))) return;
...
function closeCartPanel(fromPop = false) {  
  if (!fromPop && syncHistoryOnClose(() => closeCartPanel(true))) return;
...
function bindProductCards() {  
  ...
  cards.forEach((productEl) => {  
    ...
    const inStock = isProductInStock(productData);  

    const updateAddBtn = () => {  
      if (!addToCartBtn) return;  
      if (!inStock) { addToCartBtn.disabled = true; return; }
    ...
    addToCartBtn?.addEventListener("click", (e) => {  
      ...
      const selectedSize = productEl.querySelector(".size-btn.selected");  
      const selectedColor = productEl.querySelector(".color.selected");  
      if (!inStock) return showCartToast("This item is currently out of stock.");
...
async function renderProductRoute(id) {  
  ...
  if (rpp.label) {
    rpp.label.textContent = getProductLabel(prod);
    rpp.label.classList.toggle("out-of-stock-label", !isProductInStock(prod));
  }  
  ...
  if (rpp.add) {
    ...
    rpp.add.textContent = isProductInStock(prod) ? "Add to Bag" : "Out of Stock";
  }  
...
rpp.add?.addEventListener("click", () => {  
  ...
  const selectedSize = rpp.sizes?.querySelector(".size-btn.selected");  
  const selectedColor = rpp.colorsRow?.querySelector(".color.selected");  
  if (!isProductInStock(rppProduct)) return showCartToast("This item is currently out of stock.");
  if (!selectedSize) return showCartToast("Select a size first.");  
  if (!selectedColor) return showCartToast("Select a color first.");  
...
window.addEventListener("popstate", () => {  
  if (suppressNextPopHandler) return;
